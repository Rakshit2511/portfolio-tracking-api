...
public class HoldingUtil {
    ...
    /**
     * Private Constructor to prevent creation of objects of utility class
     */
    private HoldingUtil() {
    }
     ...
     *
     * @param holding Holding object to be updated
     * @param trade   Trade object to be added
     * @return Holding object after updating it's price and shares
     */
     ...
     *
     * @param holding Holding object to be updated
     * @param trade   Trade object to be deleted
     * @return Holding object after updating it's price and shares
     */
     ...
     *
     * @param oldHolding Holding object to be updated
     * @param newHolding Holding object to be updated
     * @param oldTrade   Trade object to be deleted
     * @param newTrade   Trade object to be added
     * @return List of Holding object after updating it's price and shares
     */
    ...
    public static List<Holding> getHoldingsFromTrades(List<Trade> trades) {
        return trades.stream()
                ...
                .values()
                .stream()
                .map(HoldingUtil::getTradesCombinedByTicker)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .filter(trade -> !BigInteger.ZERO.equals(trade.getShares()))
                .map(HoldingUtil::getHoldings)
     ...
     * Both trades should be of same ticker
     *
     * @param tradeType Tells whether trade is to be added, updated or deleted
     * @param holding   Holding to be updated
     * @param trade     Trade to be added, deleted or updated
     * @param newTrade  Trade to be updated with. It will not be null for update and will be null for add or delete
     * @return Holding after updating it's price and shares
     */
    private static Holding updateTradeInHolding(
            TradeType tradeType,
            Holding holding,
            Trade trade,
            Trade newTrade
    ...
    private static Optional<Trade> getTradesCombinedByTicker(List<Trade> trades) {
        return trades.stream()
                ...
                .values()
                .stream()
                .map(HoldingUtil::getTradesCombinedByTransactionType)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .reduce((t1, t2) -> {
                    Trade trade1, trade2;
